...
from ctrando.common import ctenums, ctrom, cttypes as ctt
...
class StatBlock(ctt.BinaryData):
    ...
    @classmethod
    def get_base_stat_offset_dict(cls) -> dict[PCStat, int]:
        return dict(cls._base_stat_offset_dict)

    @classmethod
    def get_cur_stat_offset_dict(cls) -> dict[PCStat, int]:
        return dict(cls._cur_stat_offset_dict)
...
class XPThreshholds(ctt.BinaryData):
    ...
    def set_xp_for_level(self, level: int, xp_amount):
        """
        Set the amount of xp needed to go from level-1 to level.
        """
        self[2 * level: 2 * level + 2] = xp_amount.to_bytes(2, 'little')

    def get_cum_xp_for_level(self, level: int) -> int:
        """
...
class StatGrowth(ctt.BinaryData):
    ...
    @classmethod
    def get_offset_dict(cls) -> dict[PCStat, int]:
        """Returns offsets into stat growth"""
        return dict(cls._stat_offset_dict)
...
class TechLevelRW(ctt.LocalPointerRW):
    ...
    def write_data_to_ct_rom(self, ct_rom: ctrom.CTRom,
        ...
        ct_rom.seek(start + record_num)
        ct_rom.write(data[0:1])
        # Write techs learned bitfield
        ct_rom.seek(start + record_num + 7)
        ct_rom.write(self.int_to_bitfield(data[0]), mark_used)
...
class PCStatData:
    ...
    def __init__(self,
        ...
        self.tp_thresholds = TPThresholds(tp_thresholds)
    ...
    def read_from_ctrom(cls, ct_rom: ctrom.CTRom,
        ...
        tp_thresholds = TPThresholds.read_from_ctrom(ct_rom, pc_id)

        return PCStatData(stat_block, stat_growth, hp_growth, mp_growth, tech_level, tp_thresholds)
    ...
    def write_to_ctrom(self, ct_rom: ctrom.CTRom, pc_id: ctenums.CharID):
        ...
        self.tp_thresholds.write_to_ctrom(ct_rom, pc_id)

    def get_spoiler_string(
            self,
            item_db: Optional[itemdata.ItemDB] = None) -> str:
        """
        Returns a formatted multi-line string of the stats in this object.
...
class PCStatsManager:
            ...
            for pc_id in list(ctenums.CharID):
                if pc_id in pc_stat_dict:
                    ...
                    self.pc_stat_dict[pc_id] = \
                        PCStatData(pc_stats.stat_block, pc_stats.stat_growth, pc_stats.hp_growth, pc_stats.mp_growth,
                                   pc_stats.tech_level, pc_stats.tp_thresholds)
    ...
    @classmethod
    def read_from_ct_rom(cls, ct_rom: ctrom.CTRom) -> PCStatsManager:
    ...
    def set_tech_level(self, pc_id: ctenums.CharID, new_tech_level: int):
        ...
        if new_tech_level >= 8:
            ...
            stat_block.tp_to_next_level = \
                stat_data.tp_thresholds.get_threshold(new_tech_level)
