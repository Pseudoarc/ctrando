...
def decompress(rom, start):
    ...
    main_len = int.from_bytes(rom[start:start + 2], "little")
    ...
    while True:
        ...
        if src_pos == end_pos:
            ...
            if rom[src_pos] & 0x3F == 0:
                ...
                end_pos = \
                    start + int.from_bytes(rom[src_pos+1:src_pos+3], "little")
                # print(' '.join(f"{x:02X}"
        ...
        for i in range(8):
            if src_pos == end_pos:
                ...
                copy_off = int.from_bytes(rom[src_pos:src_pos+2], "little")
...
def get_compressed_length(rom: ByteString, addr: int):
    ...
    main_length = int.from_bytes(rom[addr:addr+2], "little")
    ...
    while rom[add_byte_addr] & 0x3F != 0:
        ...
        compr_len = int.from_bytes(rom[add_byte_addr+1:add_byte_addr+3], "little")
        # print(' '.join(f"{x:02X}"
...
def decompress_geiger(rom, nStartAddr):
    ...
    # First two bytes are little endian size of compressed packet
    nCompressedSize = int.from_bytes(rom[nStartAddr:nStartAddr+2], "little")
    ...
    while True:
        ...
        if nBytePos == nByteAfter:
            ...
            nByteAfter = (nStartAddr
                          + int.from_bytes(rom[nBytePos+1:nBytePos+3]), "little")
...
def CTCopyBytes(rom, WorkingBuffer, nBytePos, nWorkPos,  bSmallerBitWidth):
    ...
    nBytesCopyNum += 2
    nBytesCopyOff = int.from_bytes(rom[nBytePos:nBytePos+2], "little")
