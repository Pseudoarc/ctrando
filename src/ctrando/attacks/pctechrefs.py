...
from ctrando.common.byteops import to_little_endian, to_rom_ptr, change_ptrs

def get_control_start_from_rom(rom: ByteString) -> int:
    """
    Return the start of control headers on the rom.
    """
    return int.from_bytes(rom[0x01CBA1: 0x01CBA1+3], "little")

def fix_control_refs(rom: ByteString, control_new_start: int):
    """
    Set the rom to read control header data from the given address.
    - control_new_start should be a
    """
...
def fix_target_refs(rom, target_new_start):
    ...
    # Experimentally determined location of pointers to targeting data
