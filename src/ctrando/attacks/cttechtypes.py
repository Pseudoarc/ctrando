...
from ctrando.common import byteops, ctenums, ctrom, cttypes as ctt
from ctrando.common.freespace import FSWriteType as FSW
from ctrando.common.cttypes import SizedBinaryData, byte_prop, bytes_prop
...
EffectMod = ctenums.WeaponEffects
...
class PCTechLearnRW(ctt.RomRW):
    ...
    @staticmethod
    def get_learn_ref_start(rom: typing.ByteString) -> int:
        ref_local_ptr = int.from_bytes(rom[0x01F261:0x01F261 + 2], 'little')
        ref_bank = rom[0x01F26A + 3] * 0x10000

        return byteops.to_file_ptr(ref_bank + ref_local_ptr)

    @staticmethod
    def get_learn_requirment_start(rom: typing.ByteString) -> int:

        lrn_ref_ptr = PCTechLearnRW.get_learn_ref_start(rom)
    ...
    @staticmethod
    def get_record_start(rom: typing.ByteString,
                         record_size: int = 3,
    ...
    def write_data_to_ct_rom(self,
        ...
        """
        record_start = PCTechLearnRW.get_record_start(
            ct_rom.getbuffer(), len(data), record_num)
        ct_rom.seek(record_start)
...
class PCTechLearnReferences(SizedBinaryData):
    ROM_RW = ctt.LocalPointerRW(0x01F26D, 0x01F261)
    SIZE = 5

    bitmask = ctt.byte_prop(0)
    first_tech_id = ctt.byte_prop(1)
    num_techs = ctt.byte_prop(2)
    offset = ctt.bytes_prop(3, 2)
...
class PCEffectMP(ctt.SizedBinaryData):
    SIZE = 1
...
def get_total_tech_count(ct_rom: ctrom.CTRom) -> int:
    ...
    magus_atk_id = ct_rom.getbuffer()[0x0C2589]
...
def get_pc_effect_header_count(
        ct_rom: ctrom.CTRom,
        num_techs: typing.Optional[int] = None
) -> int:
    """
    Determine number of effect headers.  If num_techs is left None,
    the number of techs will be computed.
    """
    ...
    max_eff = 0
    for ind in range(num_techs):
        control = PCTechControlHeader.read_from_ctrom(ct_rom, ind)
        max_eff = max(
            max(control.get_effect_index(eff_ind) for eff_ind in range(3)),
            max_eff
        )
...
def get_pc_battle_group_count(
        ct_rom: ctrom.CTRom,
        num_techs: typing.Optional[int] = None
):
    """
    Determine number of battle groups.  If num_techs is None it will be
    computed.
    """
    ...
    max_bat_group = max(
        PCTechControlHeader.read_from_ctrom(ct_rom, ind).battle_group_id
        for ind in range(num_techs)
    )

    return max_bat_group


def get_pc_menu_group_count(
        ct_rom: ctrom.CTRom
) -> int:
    return ct_rom.getbuffer()[0x3ff864] + 1
...
def get_triple_tech_count(ct_rom: ctrom.CTRom):
    ...
    num_trip_techs = ct_rom.getbuffer()[0x3FF937]
    return num_trip_techs


def get_rock_tech_count(ct_rom: ctrom.CTRom) -> int:
    return ct_rom.getbuffer()[0x3FF9B5]
...
class MenuMPReqRW(ctt.RomRW):
    ...
    @classmethod
    def get_data_start(cls, ct_rom: ctrom.CTRom) -> int:
        rom = ct_rom
    ...
    def read_data_from_ctrom(self,
        ...
        if num_bytes is not None and size != num_bytes:
            raise ValueError(
...
class PCTechDescriptionPointer(SizedBinaryData):
    ...
    pointer = bytes_prop(0, 2, 0xFFFF, 'little', ret_type=int)
...
class PCTechATBPenaltyRW(ctt.AbsPointerRW):
    ...
    def write_data_to_ct_rom(self, ct_rom: ctrom.CTRom,
        ...
        ct_rom.seek(record_start)
        ct_rom.write(data[0:1], FSW.MARK_USED)

        if num_bytes == 2:
            ct_rom.seek(record_start + self.num_triple_techs)
            ct_rom.write(data[1:2], FSW.MARK_USED)
