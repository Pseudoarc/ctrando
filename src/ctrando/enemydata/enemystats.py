...
_battle_ram_offset_dict: dict[EnemyStat, int] = {
    EnemyStat.CURRENT_HP: 3,
    EnemyStat.MAX_HP: 5,
    EnemyStat.LEVEL: 0x12,
}
...
class EnemySpriteData:
    ...
    @classmethod
    def from_rom(cls, rom: ByteString, enemy_id: int):
    ...
    def write_to_ctrom(self, ct_rom: ctrom.CTRom, enemy_id):
        ...
        self.write_to_rom(ct_rom.getbuffer(), enemy_id)
...
class EnemyStats:
    ...
    def __init__(self,
                 stat_data: Optional[ByteString] = None,
                 name_bytes: Optional[ByteString] = None,
                 reward_bytes: Optional[ByteString] = None,
        ...
        if stat_data is None:
            stat_data = bytes([0 for _ in range(0x17)])
        ...
        if reward_bytes is None:
            reward_bytes = bytes([0 for _ in range(7)])
    ...
    def get_copy(self) -> EnemyStats:
        ...
        return EnemyStats(self._stat_data,
                          self._name_bytes,
                          self._reward_data,
    ...
    @classmethod
    def from_rom(cls, rom: ByteString, enemy_id: ctenums.EnemyID):
    ...
    def from_ctrom(cls, ct_rom: ctrom.CTRom, enemy_id: ctenums.EnemyID):
        ...
        return cls.from_rom(ct_rom.getbuffer(), enemy_id)

    def write_to_ctrom(self, ct_rom: ctrom.CTRom, enemy_id: ctenums.EnemyID):
        """Write enemy stats to a CTRom."""
        ct_rom.seek(0x0C4700 + 0x17*enemy_id)
        ct_rom.write(self._stat_data)

        ct_rom.seek(0x0C6500 + 0xB*enemy_id)
        ct_rom.write(self._name_bytes)

        ct_rom.seek(0x0C5E00 + 7*enemy_id)
        ct_rom.write(self._reward_data)

        # self.sprite_data.write_to_ctrom(ct_rom, enemy_id)

        ct_rom.seek(0x21DE80+enemy_id)
        ct_rom.write(self.hide_name.to_bytes(1, 'little'))

    def _set_stats(self, stat_bytes: ByteString):
    ...
    def _set_rewards(self, reward_bytes: ByteString):
    ...
    def name(self) -> str:
        ...
        return str(self._name_bytes)

    @name.setter
    def name(self, string):
        ct_string = ctstrings.CTNameString.from_string(string)
...
def get_sprite_dict_from_ctrom(
    ...
    sprite_dict = {
        enemy_id: EnemySpriteData.from_rom(ct_rom.getbuffer(),
                                           enemy_id)
...
def set_enemy_sightscope_settings(
        ct_rom: ctrom.CTRom,
        stat_dict: dict[ctenums.EnemyID, EnemyStats],
        sightscope_all: bool,
        forced_sightscope: bool,
):
    if sightscope_all:
        for enemy_id in ctenums.EnemyID:
            stat_dict[enemy_id].can_sightscope = True
            stat_dict[enemy_id].hide_name = False

    if forced_sightscope:
        ct_rom.seek(0x0CF039)
        ct_rom.write(b'\x80')  # Change branch to BRA for always sightscope


def main():

    ct_rom = ctrom.CTRom.from_file("../ct.sfc")
    stat_dict = get_stat_dict_from_rom(ct_rom.getbuffer())

    for enemy_id in ctenums.EnemyID:
        stats = stat_dict[enemy_id]
        if stats.tp != 0:
            print(f"{enemy_id:02X} - {stats.name}: {stats.level} {stats.tp}")
