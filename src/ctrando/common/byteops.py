...
def update_ptrs(rom, ptr_list, old_start, new_start):
    for ptr in ptr_list:
        addr = int.from_bytes(rom[ptr:ptr+3], "little")
...
def update_ptrs_bytesio(rom: BytesIO, ptr_list: list[int],
                        old_start: int, new_start: int):
    """
    Shift pointers when a block of data moves.  Starts are file addresses.

    This is preferred for CTRom b/c it will mirror.
    """
    for ptr in ptr_list:
        rom.seek(ptr)
        addr = int.from_bytes(rom.read(3), 'little')
        addr = to_file_ptr(addr)
        ...
        rom.seek(ptr)
        try:
            rom.write(new_ptr.to_bytes(3, 'little'))
        except OverflowError:
            print(f'{ptr:06X}, {addr:06X}, os={old_start:06X}, '
                  f'ns={new_start:06X}')
            raise OverflowError
...
def file_ptr_from_rom(rom, addr, num_bytes=3):
    return to_file_ptr(int.from_bytes(rom[addr:addr+num_bytes], "little"))
...
def get_maximal_shift(mask: int):
    """
    Find the minimal N such that (mask >> N) == 0.
    """
    
    # Put the binary string in reverse, and chop off the '0b' from the
    # start.
    try:
        return len(bin(mask))-2
    except ValueError as orig:  # Give a more reasonable ValueError
        raise ValueError('Mask must be nonzero.') from orig


def get_masked_range(data: typing.ByteString, start_idx: int, num_bytes: int,
                     mask: int,
    ...
    if is_signed:
        max_shift = get_maximal_shift(mask)
        if val & (1 << (max_shift-1)):
            val -= (1 << max_shift)
    return val
...
def set_masked_range(data: bytearray, start_idx: int, num_bytes: int,
    ...
    if val not in range(min_val, max_val+1):
        raise ValueError(
            f'Value must be in range({min_val:0{2*num_bytes}}, '
            f'{max_val+1:0{2*num_bytes}X})'
        )
    ...
    data[start_idx:start_idx+num_bytes] = \
        cur_val.to_bytes(num_bytes, byteorder, signed=is_signed)


def get_string(buf: typing.ByteString, pos: int = 0, terminator: int=0):
    """Extract a terminated string from the buffer."""

    length = get_string_length(buf, pos, terminator)
    return bytes(buf[pos:pos+length])

def get_string_length(buf: typing.ByteString, pos: int = 0, terminator: int=0):
    """Determine the length of a terminated string from the buffer.  Includes terminator."""

    chunk_size = 0x100
    end = pos

    while True:
        chunk = bytes(buf[end:end+chunk_size])

        try:
            terminator_loc = chunk.index(terminator)
            end = end + terminator_loc
            break
        except ValueError:
            if end == len(buf):
                raise ValueError("Could not find terminator")

            if end + chunk_size > len(buf):
                end = len(buf)
            else:
                end += chunk_size
