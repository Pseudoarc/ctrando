...
from ctrando.common.memory import Memory as Mem
from ctrando.common.byteops import to_little_endian
...
class Operation(IntEnum):
    ...
    BITWISE_AND_ZERO = 7  # BITWISE_OR_NONZERO = 7  changed in basepatch
...
class EventCommand:
    ...
    def set_explore_mode(is_on: bool) -> EventCommand:
        ...
        # ret_cmd = copy.copy(event_commands[0xE3])
        ret_cmd.args = [0]
        if is_on:
            ret_cmd.args[0] = 1
        else:
    ...
    @staticmethod
    def follow_pc_once(pc_id: int):
        """Follow the given pc (party id)"""
        return EventCommand.generic_command(0x95, pc_id)

    @staticmethod
    def follow_obj_once(obj_id: int):
        """Follow the given object once."""
        return EventCommand.generic_command(0x94, 2*obj_id)

    @staticmethod
    def follow_obj_infinite(obj_id: int) -> EventCommand:
        """Follow the given object always."""
        return EventCommand.generic_command(0xB5, obj_id*2)

    @staticmethod
    def move_party(pc1_x, pc1_y, pc2_x, pc2_y, pc3_x, pc3_y) -> EventCommand:
        ret_cmd = event_commands[0xD9].copy()
        # ret_cmd = copy.copy(event_commands[0xD9])
    ...
    def change_location(location, x_coord, y_coord, facing=0,
        ...
        ret_cmd.args[0] = (facing & 0x03) << 0x9
    ...
    @staticmethod
    def add(address: int, val: int) -> EventCommand:
        """
        Adds val to the value in (script) address.
        """

        offset = get_offset(address)
        return EventCommand.generic_command(0x5B, val, offset)

    @staticmethod
    def add_mem(add_from_addr: int, add_to_addr: int, num_bytes: int = 1) -> EventCommand:
        """Add from script memory"""
        offset_from = get_offset(add_from_addr)
        offset_to = get_offset(add_to_addr)

        cmd_id = 0x5D
        if num_bytes == 2:
            cmd_id = 0x5E

        return EventCommand.generic_command(cmd_id, offset_from, offset_to)

    @staticmethod
    def sub(address: int, val: int, num_bytes: int = 1) -> EventCommand:
        """
        Subtracts val from the value in (script) address
        """
        offset = get_offset(address)
        if num_bytes == 1:
            return EventCommand.generic_command(0x5F, val, offset)

        return EventCommand.generic_command(0x60, val, offset)
    ...
    @staticmethod
    def reset_byte(address: int) -> EventCommand:
        """
        Set an address (script memory) to 0.
        """
        offset = get_offset(address)
        return EventCommand.generic_command(0x77, offset)

    @staticmethod
    def set_byte(address: int) -> EventCommand:
        """Set an address of script memory to 1."""
        offset = get_offset(address)
        return EventCommand.generic_command(0x75, offset)

    @staticmethod
    def set_flag(flag: memory.Flags):
        """Set a flag"""
        flag_data = flag.value
        if isinstance(flag_data, memory.FlagData):
            return EventCommand.set_bit(flag_data.address, flag_data.bit)
        raise TypeError

    @staticmethod
    def reset_flag(flag: memory.Flags):
        """Reset a flag"""
        flag_data = flag.value
        if isinstance(flag_data, memory.FlagData):
            return EventCommand.reset_bit(flag_data.address, flag_data.bit)
        raise TypeError
    ...
    @staticmethod
    def set_object_script_processing(obj_id: int,
                                     is_processing: bool) -> EventCommand:
        """
        Turn object script processing on or off
        """
        ...
        return EventCommand.generic_command(cmd_id, obj_id*2)
    
    @staticmethod
    def vector_move(angle: float, magnitude: int,
                    keep_facing: bool) -> EventCommand:
        hex_angle = int((0x100 * angle)//360)
        cmd_mag = magnitude*2
    ...
    def call_obj_function(obj_id: int,
        ...
        # ret =copy.copy(event_commands[cmd_id])
    ...
    def copy_tiles(src_left: int, src_top: int, src_right: int, src_bot: int,
                   ...
                   unk_0x10: bool = False,
                   unk_0x20: bool = False,
                   copy_z_plane: bool = False,
                   copy_wind: bool = False,
    ...
    def get_blank_command(cmd_id: int) -> EventCommand:
        ...
        # ret_cmd = copy.copy(event_commands[cmd_id])
        ret_cmd.args = [0 for i in range(ret_cmd.num_args)]
        return ret_cmd
    ...
    def generic_command(*args) -> EventCommand:
        ...
        # ret_cmd = copy.copy(event_commands[args[0]])
    ...
    def generic_zero_arg(cmd_id: int) -> EventCommand:
        ...
        ret = event_commands[cmd_id].copy()
        return ret
        # return copy.copy(event_commands[cmd_id])
    ...
    @staticmethod
    def equip_item_to_pc(
            pc_id: ctenums.CharID,
            item_id: ctenums.ItemID
    ) -> EventCommand:
        """
        Returns a command which equips an item to a PC.
        """
        return EventCommand.generic_command(0xD5, pc_id, item_id)
    ...
    @staticmethod
    def if_storyline_counter_lt(storyline_val: int, jump_bytes: int = 0):
        return EventCommand.generic_command(0x18, storyline_val, jump_bytes)

    @staticmethod
    def if_has_item(item_id: int, jump_bytes: int = 0) -> EventCommand:
        return EventCommand.generic_two_arg(0xC9, int(item_id), jump_bytes)

    @staticmethod
    def if_has_gold(gold_amt: int, jump_bytes: int = 0) -> EventCommand:
        """
        jumps if the player does has gold < gold_amt.
        """
        return EventCommand.generic_command(0xCC, gold_amt, jump_bytes)

    @staticmethod
    def if_pc_active(pc_id: int, jump_bytes: int = 0) -> EventCommand:
        """Branch command for if a pc is active."""
        return EventCommand.generic_command(0xD2, pc_id, jump_bytes)

    @staticmethod
    def if_pc_recruited(pc_id: int, jump_bytes: int = 0) -> EventCommand:
        """Branch command for if a pc is recruited."""
        return EventCommand.generic_command(0xCF, pc_id, jump_bytes)

    @staticmethod
    def if_mem_op_value(
            address: int, operation: Operation,
            value: int, num_bytes: int = 1,  bytes_jump: int = 0
    ) -> EventCommand:
    ...
    @staticmethod
    def if_mem_op_mem(
            lhs_addr: int, operation: Operation, rhs_addr: int,
            num_bytes: int = 1, bytes_jump: int = 0
    ) -> EventCommand:
        """
        Returns an event command which branches if the lhs OP rhs is false.
        """
        lhs_offset = get_offset(lhs_addr)
        rhs_offset = get_offset(rhs_addr)
        ...
        return EventCommand.generic_command(cmd_id, lhs_offset, rhs_offset,
                                            operation, bytes_jump)

    @staticmethod
    def set_storyline_counter(val: int) -> EventCommand:
        # return EventCommand.assign_val_to_mem(val, 0x7F0000, 1)
        return EventCommand.generic_command(0x5A, val)
    ...
    def add_value_to_mem(value: int, script_addr: int):
        ...
        # cmd = copy.copy(event_commands[0x5B])
    ...
    def assign_mem_to_mem(
        ...
        # cmd = copy.copy(event_commands[cmd_id])
        cmd.args = cmd_args
    ...
    def assign_val_to_mem(
        ...
        if 0x7F0200 <= address <= 0x7F03FF and address % 2 == 0:
            ...
            # out_cmd = copy.copy(event_commands[cmd_id])
            ...
            if num_bytes == 1:
                ...
                # out_cmd = copy.copy(event_commands[0x56])
                offset = (address - 0x7F0000)
                out_cmd.args = [val, offset]
            else:
                out_cmd = event_commands[0x4B].copy()
                # out_cmd = copy.copy(event_commands[0x4B])
    ...
    @staticmethod
    def if_flagdata(flagdata: memory.FlagData, jump_bytes: int = 0):
        return EventCommand.if_mem_op_value(
            flagdata.address, Operation.BITWISE_AND_NONZERO,
            flagdata.bit, 1, jump_bytes)

    @staticmethod
    def if_flag(flag: memory.Flags, bytes_jump: int = 0) -> EventCommand:
        flag_val = flag.value

        if isinstance(flag_val, memory.FlagData):
            return EventCommand.if_flagdata(flag_val, bytes_jump)

        raise TypeError

    @staticmethod
    def if_not_flagdata(flagdata: memory.FlagData, bytes_jump: int = 0):
        return EventCommand.if_mem_op_value(
            flagdata.address, Operation.BITWISE_AND_ZERO,
            flagdata.bit, 1, bytes_jump)

    @staticmethod
    def if_not_flag(flag: memory.Flags, bytes_jump: int = 0) -> EventCommand:
        flag_val = flag.value

        if isinstance(flag_val, memory.FlagData):
            return EventCommand.if_not_flagdata(flag_val, bytes_jump)

        raise TypeError

    @staticmethod
    def lock_chars(
            locked_chars: ctenums.CharID | list[ctenums.CharID]
    ) -> EventCommand:
        """Produce a command to write to the charlock byte"""
        lock_byte = 0x00
        if isinstance(locked_chars, ctenums.CharID):
            locked_chars = [locked_chars]
        
        for char_id in locked_chars:
            lock_byte |= 0x80 >> char_id

        return EventCommand.assign_val_to_mem(
            lock_byte, Mem.CHARLOCK, 1
        )
    ...
    @staticmethod
    def jump_back(jump_bytes: int = 0) -> EventCommand:
        return EventCommand.generic_one_arg(0x11, jump_bytes)

    @staticmethod
    def jump_forward(jump_bytes: int = 0) -> EventCommand:
    ...
    @staticmethod
    def move_pc_to_reserve(char_id: int) -> EventCommand:
        """
        Moves an active pc to reserve.  This works even if the reserve party
        is full (will or with 0x80).

        Really, this seems like it mangles the party memory a bit.  Avoid.
        """
        return EventCommand.generic_command(0xD4, char_id)

    @staticmethod
    def remove_pc_from_party(char_id: int) -> EventCommand:
        """Remove a PC from the party."""
        return EventCommand.generic_command(0xD1, char_id)

    @staticmethod
    def remove_pc_from_active_party(char_id: int) -> EventCommand:
        """Remove a PC from the active party."""
        return EventCommand.generic_command(0xD6, char_id)
    
    @staticmethod
    def get_object_coordinates(obj_id: int,
                               x_addr: int,
                               y_addr: int) -> EventCommand:
        """Put the given object's coordinates into script memory."""
        return EventCommand.generic_command(
            0x21, obj_id*2,
            get_offset(x_addr),
            get_offset(y_addr)
    ...
    def get_pc_coordinates(pc_id: int,
                           x_addr: int,
                           y_addr: int) -> EventCommand:
        """Put the given PC's coordinates into script memory."""
        return EventCommand.generic_command(
            0x22, pc_id*2, get_offset(x_addr), get_offset(y_addr)
        )

    @staticmethod
    def get_object_facing(obj_id: int, facing_addr: int) -> EventCommand:
        """Put the object's facing in the given script memory."""
        return EventCommand.generic_command(0x23, obj_id*2, get_offset(facing_addr))

    @staticmethod
    def get_pc_facing(pc_id: int, facing_addr: int) -> EventCommand:
        """Put the PC's facing in the given script memory."""
        return EventCommand.generic_command(0x24, pc_id*2, get_offset(facing_addr))
    ...
    def set_object_coordinates_auto(px_x_coord: int,
                                    px_y_coord: int,
                                    force_px: bool = False) -> EventCommand:
        tile_x = px_x_coord - 0x8
        tile_y = px_y_coord - 0xF

        if not force_px and ((tile_x & 0xF) == 0 and (tile_y & 0xF) == 0):
            tile_x >>= 4
            tile_y >>= 4
    ...
    @staticmethod
    def set_object_facing(obj_id: int,
    ...
    @staticmethod
    def get_result(script_addr: int) -> EventCommand:
        """
        Assign a script address to the result address.
        """
        offset = get_offset(script_addr)
        return EventCommand.generic_command(0x19, offset)

    @staticmethod
    def if_result_equals(result_val: int, jump_bytes = 0) -> EventCommand:
    ...
    @staticmethod
    def set_solidity_properties(
            is_solid: bool = True, is_pushable: bool = False
    ) -> EventCommand:
        return EventCommand.generic_command(0x84, is_solid | is_pushable << 1 )

    @staticmethod
    def set_move_speed(speed: int) -> EventCommand:
        """Movement speed of an object."""
        return EventCommand.generic_command(0x89, speed)

    @staticmethod
    def set_move_properties(through_wall: bool,
                            through_pc: bool) -> EventCommand:
        """Set NPC movement properties"""
        through_wall = not bool(through_wall)
        through_pc = not bool(through_pc)

        prop_byte = through_wall | (through_wall << 1)
        return EventCommand.generic_command(0xD, prop_byte)

    @staticmethod
    def set_move_destination(on_tile: bool,
                             on_obj: bool) -> EventCommand:
        """Set how NPCs move to coordinates/objects."""
        on_tile = bool(on_tile)
        on_obj = bool(on_obj)

        prop_byte = (on_obj << 1) | on_tile
        return EventCommand.generic_command(0xE, prop_byte)

    @staticmethod
    def move_sprite(x_coord: int,
                    y_coord: int,
                    is_animated: bool = False) -> EventCommand:
        """Move sprite (tile-based)"""
        if is_animated:
            return EventCommand.generic_command(0xA0, x_coord, y_coord)

        return EventCommand.generic_command(0x96, x_coord, y_coord)

    @staticmethod
    def move_sprite_offsets(x_address: int,
                            y_address: int,
                            is_animated: bool = False) -> EventCommand:
        """
        Move Sprite to offsets (tile based)
        """
        x_offset, y_offset = get_offset(x_address), get_offset(y_address)
        if is_animated:
            return EventCommand.generic_command(0xA1, x_offset, y_offset)

        return EventCommand.generic_command(0x97, x_offset, y_offset)

    @staticmethod
    def set_own_facing_object(obj_id: int) -> EventCommand:
        """Make object face the given object."""
        return EventCommand.generic_command(0xA8, obj_id*2)
    
    @staticmethod
    def set_own_facing_pc(pc_id: int) -> EventCommand:
        """Make object face the given pc"""
        return EventCommand.generic_command(0xA9, pc_id*2)
    
    @staticmethod
    def set_own_facing(
            direction: Literal['up', 'down', 'left', 'right']
    ) -> EventCommand:
        """Used by an object to set its own facing."""
        if direction == 'up':
            cmd_id = 0x0F
        elif direction == 'down':
            cmd_id = 0x17
        ...
        return EventCommand.generic_command(cmd_id)

    @staticmethod
    def shake_screen(is_shaking: bool = False):
        """Turn screen shaking on/off"""
        return EventCommand.generic_command(0xF4, int(is_shaking))

    @staticmethod
    def reset_animation() -> EventCommand:
        """Reset object animation status."""
        return EventCommand.generic_command(0xAE)

    @staticmethod
    def play_animation(anim_id, is_looping: bool = False):
        """Play a normal animation, possibly looping"""
        if is_looping:
            cmd_id = 0xAB
        else:
            cmd_id = 0xAA

        return EventCommand.generic_command(cmd_id, anim_id)
    
    @staticmethod
    def loop_animation(anim_id: int, num_loops: int) -> EventCommand:
        """play a looping animation"""
        return EventCommand.generic_command(0xB7, anim_id, num_loops)

    @staticmethod
    def static_animation(anim_id: int) -> EventCommand:
        """play a static animation"""
        return EventCommand.generic_command(0xAC, anim_id)
    
    @staticmethod
    def play_song(song_id: int) -> EventCommand:
        """Play the given song."""
        return EventCommand.generic_command(0xEA, song_id)

    @staticmethod
    def play_sound(sound_id: int) -> EventCommand:
        """Play the given sound."""
        return EventCommand.generic_command(0xE8, sound_id)
    ...
    @staticmethod
    def disable_script_processing(obj_id: int) -> EventCommand:
        """Disable scripts on an object"""
        return EventCommand.generic_command(0xB, obj_id*2)

    @staticmethod
    def enable_script_processing(obj_id: int) -> EventCommand:
        """Enable scripts on an object"""
        return EventCommand.generic_command(0xC, obj_id*2)
    ...
    @staticmethod
    def open_portal_scene(screen_x_px, screen_y_px, radius_px = 0x20):
        """
        Play a portal at the given coordinates.  Note that this only plays
        the mode7 scene which (I think) defines a circular region in which L3
        will be shown over L1/L2.  The user still has to put the appropriate
        objects in L3.
        """
        if not (
                screen_x_px in range(0x100) and
                screen_y_px in range(0x100) and
                radius_px in range(0x100)
        ):
            raise ValueError("x, y, and radius must fit in one byte.")

        cmd_list: list[int] = [0xFF, 0x90, screen_x_px, screen_y_px, radius_px]
    ...
    def set_level(pc_id, level: int):
        """
        Returns a command that sets a PC's level.
        """
        if not 0 <= pc_id < 7:
            raise ValueError
        ...
        return EventCommand.generic_command(0xFC, pc_id, level)

    @staticmethod
    def set_level_from_memory(pc_id, address: int):
        """
        Returns a command that sets a PC's level from the script memory in address.
        """
        if not 0 <= pc_id < 7:
            raise ValueError
        ...
        # return EventCommand.return_cmd()
        return EventCommand.generic_command(0xFC, pc_id | 0x80, offset)

    @staticmethod
    def set_tech_level(pc_id: int, tech_level: int):
        """
        Returns a command that set's a PC's tech level.
        """
        if not 0 <= pc_id < 7:
            raise ValueError

        if not 0 <= tech_level < 9:
            raise ValueError

        # return EventCommand.return_cmd()
        return EventCommand.generic_command(0xFD, pc_id, tech_level)

    @staticmethod
    def set_tech_level_from_memory(pc_id: int, tech_level_addr: int) -> EventCommand:
        """
        Returns a set_tech_level command where the tech level is in script memory.
        """
        if not 0 <= pc_id < 7:
            raise ValueError

        # return EventCommand.return_cmd()
        return EventCommand.generic_command(0xFD,pc_id | 0x80, get_offset(tech_level_addr))
...
event_commands[0xFC] = \
    EventCommand(0xFC, 2, [1, 1],
                 arg_descs=[
                     "cc: pc_id of character.  If 0x80 set, use ll as script offset",
                     "ll: level to set.  Or If 0x80 of cc is set, read the level from"
                     " 0x7F0200 + 2*ll"
                 ],
                 name="Set Level",
                 desc="Set Level")
...
event_commands[0xFD] = \
    EventCommand(0xFD, 2, [1, 1],
                 arg_descs=['cc: pc_id of character',
                            'll: new tech level'],
                 name="Set Tech Level",
                 desc="Set Tech Level")
...
def get_command(buf: bytes, offset: int = 0) -> EventCommand:
    ...
    # command = copy.copy(event_commands[command_id])
    ...
    if command_id == 0x2E:
        ...
        if mode in [4, 5]:
            ...
            data_len_b = buf[offset + 3: offset+5]
            data_len = int.from_bytes(data_len_b, "little") - 2
            command.arg_lens = [1, 1, 2, data_len]
        else:
            raise ValueError(f"{command_id:02X}: Error, Unknown Mode")
    elif command_id == 0x4E:
        # Data to copy follows command.  Shove data in last arg.
        data_len = int.from_bytes(buf[offset+4:offset+6], "little") - 2
    ...
    if command.command == 0x4E:
        for i in command.arg_lens[0:-1]:
            command.args.append(int.from_bytes(buf[pos:pos+i], "little"))
            pos += i
