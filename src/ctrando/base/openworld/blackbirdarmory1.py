...
class EventMod(locationevent.LocEventMod):
    """EventMod for Blackbird Armory 1"""
    loc_id = ctenums.LocID.BLACKBIRD_ARMORY_1
    ...
    def modify(cls, script: Event):
        """
        Modify Blackbird Armory 1 for an Open World.
    ...
    def modify_gear_restoration_function(cls, script: Event):
        ...
        pos = script.find_exact_command(
            EC.if_mem_op_value(memory.Memory.BLACKBIRD_IMPRISONED_PC1, OP.EQUALS, 1),
            script.get_function_start(6, FID.ACTIVATE)
        )

        end = script.find_exact_command(
            EC.set_flag(memory.Flags.BLACKBIRD_FOUND_PC1_GEAR), pos
        )

        new_block = EF()
        for pc_id in ctenums.CharID:
            new_block.add_if(
                EC.if_mem_op_value(memory.Memory.BLACKBIRD_IMPRISONED_PC1,
                                   OP.EQUALS, pc_id),
        ...
        # This one is like Armory2 but different from Armory3
